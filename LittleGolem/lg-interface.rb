...
class Logger
	def log(msg)
		puts((('[' + Time::now.strftime('%d-%m-%y %H:%M:%S') + '] ').green) + msg)
	end
end
...
class LittleGolemInterface
	...
	def reply_invitation(inv_id,answer)
		...
		self.log("Replying to #{path}")
		resp = @http.get(path, @headers)
		if (resp.code == '200')
			self.log('Successfully replied to invitation')
			return resp.body
		else
			self.log('Failed to reply to invitation')
			return nil
		end
	end
	def log(msg)
	...
	def login
		...
		self.log("Logged in as #{@login}")
		return (resp.code == '200' ? resp.body : nil)
	end
	def get_gamesheet
		self.log('Getting gamesheet')
		path='/jsp/game/index.jsp'
		resp = @http.get(path, @headers)
		if (resp.code == '200')
			return resp.body
		else
			self.log('Could not get gamesheet')
			return nil
		end
	end
	def get_my_games
		self.log('Getting my games')
		path="/get_xml.jsp"
		...
		if self.login
			if (gamesheet = get_gamesheet)
				if !(gamesheet =~ /On Move/)
					return gamesheet.slice(/On Move.*Opponent's Move/m).scan(/gid=(\d+)?/).flatten				end  
			end
		else
	...
	def parse
		...
		if (gamesheet = get_gamesheet)
			#check invitations
			if gamesheet =~ /See all invitations/
